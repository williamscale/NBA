...
shotchart.team.bivariate.plotter <- function(data.transformed, team) {
  
  fg.halfcourt <- data.transformed %>%
    filter(LOC_Y <= 0,
           TEAM_NAME == team)

  team.name <- fg.halfcourt$TEAM_NAME[1]
...
## shotchart.team.bivariate.plotter() -------------------------------------

# load('./../Shotcharts/2021-22/RegularSeason/regularseason_2021-22.RData')
# 
# shotchart.team.bivariate.plotter(data.transformed = data,
#                                  team = 'Boston Celtics')
